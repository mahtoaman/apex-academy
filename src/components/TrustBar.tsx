...
const marqueeItemsReverse = [...partners.slice(4), ...partners.slice(0, 4), ...partners, ...partners, ...partners];
  ...
  return (
    <section ref={ref} className="py-24 relative overflow-hidden bg-background">
      ...
      <div className="relative flex flex-col gap-6 transform -rotate-1 lg:-rotate-2 scale-[1.05] overflow-hidden py-10">
        ...
        <div className="marquee flex w-fit hover:[animation-play-state:paused] gap-6 px-3 items-center">
        ...
        <div className="marquee-reverse flex w-fit hover:[animation-play-state:paused] gap-6 px-3 ml-[-50%] items-center">
          {marqueeItemsReverse.map((name, i) => (
            <div
              key={`row2-${i}`}
              className="flex-shrink-0 flex items-center justify-center px-8 py-5 rounded-2xl border border-border/50 bg-background/50 backdrop-blur-md shadow-lg transition-all duration-500 hover:scale-110 hover:-translate-y-2 hover:bg-accent/5 hover:border-accent/40 hover:shadow-accent/20 hover:z-20 cursor-default group"
            >
